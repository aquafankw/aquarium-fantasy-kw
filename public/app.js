...
function populateCategories() {
  ...
  const icons = {Animals:"🐠",Basics:"💧",Feeder:"🍽️",Figures:"🗿",Floating:"🫧",Houses:"🏠","Kids Tank":"🧒","My Box":"📦",Nature:"🌿",Plants:"🌱","Stone Type":"🪨","Tank Design":"🎨","Tank Type":"🐟",Packages:"🎁"};
  const active = select.value || "all";
  $("#categoryRail").innerHTML = [{value:"all",label:text("الكل", "All"),icon:"🌊"}, ...categories.map(category => ({value:category,label:category,icon:icons[category] || "🐚"}))].map(item => `<button type="button" class="category-circle${item.value === active ? " active" : ""}" data-category="${escapeHtml(item.value)}"><span aria-hidden="true">${item.icon}</span><b>${escapeHtml(item.label)}</b></button>`).join("");
}
...
function bindEvents() {
  ...
  const rail=$("#categoryRail"); let dragging=false,startX=0,startScroll=0,moved=false; rail.addEventListener("pointerdown",e=>{dragging=true;moved=false;startX=e.clientX;startScroll=rail.scrollLeft;rail.setPointerCapture(e.pointerId);rail.classList.add("dragging")});rail.addEventListener("pointermove",e=>{if(!dragging)return;const delta=e.clientX-startX;if(Math.abs(delta)>5)moved=true;rail.scrollLeft=startScroll-delta});rail.addEventListener("pointerup",()=>{dragging=false;rail.classList.remove("dragging")});
